...
def OLS_cross_reg(n_points=20, degrees=5, k=5, scaling=False, noisy=True, r_seed=9): 
    ...
    for degree in range(1, degrees+1): 
        pred_train_avg = np.empty((k, ))
        pred_test_avg = np.empty((k, ))
        training_error = 0 
        test_error = 0
        ...
        i += i2 
        i2 += 1
        MSE_train[degree-1] = training_error/k #np.mean(np.mean((pred_train_avg-z_train)**2, axis=0, keepdims=True))#training_error/n_boots
        MSE_test[degree-1] = test_error/k #np.mean(np.mean((pred_test_avg-z_test)**2, axis=0, keepdims=True))#test_error/n_boots
...
def plot_OLS_boot_figs(*args):
    ...
    axs[0,0].plot(args[4], np.log10(args[0]), 'b', label='MSE_train') 
    axs[0,0].plot(args[4], np.log10(args[1]), 'r', label='MSE_test')
...
# Good values for the random seed variable r_seed => [2, 3, 
# Size of dataset good for the analysis of bias-variance trade-off => 10

bias,var, MSE_train, MSE_test, pol = OLS_cross_reg(n_points=10, degrees=11, r_seed=17, k=6)
