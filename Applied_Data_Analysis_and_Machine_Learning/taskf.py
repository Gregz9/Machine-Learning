...
def task_f(n_points=20, n_lambdas=6, r_seed=79, n_boots=100, degrees=12,       
    ...
    MSE_train_boot, MSE_test_boot, bias_, variance_, deg = Lasso_reg_boot(x, y, lambdas=lambdas, r_seed=r_seed, n_points=n_points,
    ...
    for i in range(len(folds)):
        ...
        MSE_train, MSE_test, deg = Lasso_reg_kFold(x,y,lambdas=np.array([lam]), degrees=degrees, folds=folds[i], r_seed=r_seed, scaling=centering)
    ...
    if kfold_for_all_lam: 
        MSE_train_all_folds, MSE_test_all_folds, deg = Lasso_reg_kFold(x,y,lambdas=lambdas, degrees=degrees, folds=folds[len(folds)-1], r_seed=r_seed, scaling=centering)
