...
from plot_functions import (plot_figs_bootstrap_all_lambdas, plot_kfold_figs_for_k, plot_kFold_figs_for_L, 
                            plot_compare_bootstrap_OLS_Ridge)
...
def task_f(n_points=20, n_lambdas=6, r_seed=79, n_boots=100, degrees=12,       
    ...
    lam_L, index_L = find_best_lambda(lambdas, MSE_train_boot)
    ...
    if compare:
        ...
        _, MSE_test_ridge, bis_ridge, var_ridge, _ = Ridge_reg_boot(x,y, lambdas=lambdas, n_points=n_points, degrees=degrees, n_boots=n_boots,
                                                                    noisy=noisy, r_seed=r_seed, scaling=centering)

        lam_R, index_R = find_best_lambda(lambdas, MSE_train_boot)

        plot_compare_bootstrap_OLS_Ridge(MSE_test_boot[index_L], variance_[index_L], bias_[index_L], lam_L, MSE_test_ols, var_ols, bias_ols, deg)
        ...
        for i in range(len(folds)):
            ...
            MSE_train_R, MSE_test_R, _ = Ridge_reg_kFold(x,y,n_points=n_points, noisy=noisy, degrees=degrees, 
                                                        r_seed=r_seed, folds=[folds[i]], scaling=centering)
